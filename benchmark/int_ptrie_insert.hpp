...
{
    el ^= seed;
    constexpr auto size_bits = ptrie::BitSize<>::of<size_t>();
    auto s = ptrie::binarywrapper_t{ptrie::as_array(&el), size_bits};
    auto t = ptrie::binarywrapper_t{size_bits};
    bool flip[8];
    for (size_t i = 0; i < 8; ++i)
        flip[i] = s.at(i);
    for (size_t i = 0; i < size_bits.bits(); ++i) {
        if (i < 8) {
    ...
    const size_t& target = *ptrie::as_array<size_t>(t.raw());
    return target;
}
...
void set_insert(T& set, const Settings& s, const std::vector<size_t>& order)
{
    ...
    for (size_t i = 1; i <= s.elements; ++i) {
        const size_t val = reorder(i, order, s.seed);
        set.insert(val);

        if (s.read_rate > 0.0) {
            const int reads = static_cast<int>(std::round(read_dist(read_generator)));
            for (int r = 0; r < reads; ++r) {
                const size_t el = reorder(read_el(read_generator), order, s.seed);
                set.count(el);
            }
        }
        ...
        if (dist(generator) < s.deletes) {
            ...
            const size_t el = reorder(rem(generator), order, s.seed);
...
void set_insert_ptrie(T& set, const Settings& s, const std::vector<size_t>& order)
{
    ...
    for (size_t i = 1; i <= s.elements; ++i) {
        ...
        if (del_dist(del_generator) < s.deletes) {
            ...
            set.erase(ptrie::as_array(&el), sizeof(el));
        }
    }
}
